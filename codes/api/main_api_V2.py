...
def get_player_from_list(joueur1_choisi: str, joueur2_choisi: str):
    ...
    if joueur1_choisi not in list(player_base_updated_df['Player_name']):
        raise ValueError(
            'Player 1 is not in the database. Please enter a valid player such as Federer R.')

    elif joueur2_choisi not in list(player_base_updated_df['Player_name']):
        raise ValueError(
            'Player 2 not in database. Please enter a valid player such as Nadal R.')

    else:
        p1 = list(filter(lambda x: x.get('Player_name') ==
                  joueur1_choisi, player_base_updated_db))
        p2 = list(filter(lambda x: x.get('Player_name') ==
                  joueur2_choisi, player_base_updated_db))
        return p1, p2
...
def get_tournament_from_list(tournois_choisi: str | None = 'FRENCH OPEN'):
    ...
    if tournois_choisi.upper() not in list(tournois_df['tournament']):
        ...
        t = list(filter(lambda y: y.get('tournament') ==
                 tournois_choisi.upper(), tournois_db))
        return t
...
def new_game_list(date: str = Query('2018-03-03'), player1: str | None = 'Jarry N.', player2: str | None = 'Zeballos H.'):
    ...
    specific_game = df_preprocessed[((df_preprocessed['p1_Name'] == player1) | (df_preprocessed['p2_Name'] == player1)) & (
        (df_preprocessed['p1_Name'] == player2) | (df_preprocessed['p2_Name'] == player2)) & (df_preprocessed['date'] == date)]
